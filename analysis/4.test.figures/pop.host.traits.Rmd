...
g1 <- ggplot(beta, aes(x = Description, y = R2*100, group = Site)) +
  ...
  labs(x = "Factor", y = bquote("Variation\nexplained (%)"))
g1  
```
...
asvs.per.site <- to.plot %>% 
  ...
  ungroup %>% 
  mutate(Group = if_else(Group == "Age/BMI/Sex",
                         "A./BMI/S.",
                         Group)) %>% 
  mutate(Group = if_else(Group == "Sknc.",
                         "Others",
                         Group)) %>% 
  mutate(Description = if_else(Description == "Antibiotics (<6-8w)",
                               "Antibiotics (<6w)",
...
g1.1 <- asvs.per.site %>% 
  ...
  labs (fill = "ASVs\n(n)", y = "Factor") +
  theme(legend.position = "right", strip.text.x = element_text(angle = 45), panel.spacing = unit(0.0, "lines"),
...
# Plot overview
```{r, fig.width= 10, fig.height=10.8}
g12 <- ggpubr::ggarrange(g1, g1.1, labels = c("A", "B"),  ncol = 2, widths = c(1.5,3))
g12
ggpubr::ggarrange(g12, g2, labels = c("", "C"),  nrow = 2, heights = c(1.5,1.2))
```


# Plot ASVs

```{r, fig.width=15.5, fig.height=11}
theme_set(original)
...
to.plot.selected <- to.plot %>% 
  ...
  mutate(Species = if_else(str_detect(Species, " $"),
                           "Unclassified",
                           Species)) %>%
  mutate(Genus = str_sub(Genus, 1, 5) %>% paste0(".")) %>% 
...
g3.1 <- ggplot(this.plot,
  ...
  ggrepel::geom_label_repel(data = this.plot %>% filter(!is.na(labels)), box.padding = 1, aes(color = ASV.code), size = 5) +
...
ggpubr::ggarrange(g3.1, g3.2, labels = "AUTO", nrow = 2, heights = c(9, 7), font.label = list(size = 20))
```
